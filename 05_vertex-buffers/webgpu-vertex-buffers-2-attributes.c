...
circleVertices createCircleVertices(circleVerticesConfig config) {
  ...
  for (i = 0; i < config.numSubdivisions; ++i) {
    ...
    const float innerColor[3] = {1, 1, 1};
    const float outerColor[3] = {0.1, 0.1, 0.1};
...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor shaderSource =
      load_wgsl(RESOURCE_DIR
                "/05_vertex-buffers/webgpu-vertex-buffers-2-attributes.wgsl");
