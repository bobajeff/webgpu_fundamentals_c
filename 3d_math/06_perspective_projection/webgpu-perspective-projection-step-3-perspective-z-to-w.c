...
void makeZToWMatrix(float fudgeFactor, float * dest) {
    dest[0]  = 1.0,  dest[1]  = 0.0,  dest[2]  = 0.0,   dest[3]  = 0.0,
    dest[4]  = 0.0,  dest[5]  = 1.0,  dest[6]  = 0.0,   dest[7]  = 0.0,
    dest[8]  = 0.0,  dest[9]  = 0.0,  dest[10] = 1.0,   dest[11] = fudgeFactor,
...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor shaderSource = load_wgsl(
      RESOURCE_DIR "webgpu-perspective-projection-step-3-perspective-z-to-w.wgsl");
  ...
  float translation[NUM_OF_PRESETS][3] = {
    {__window_width / 2.0 - 200.0, __window_height / 2.0 - 75.0, -1000.0},
    {200.0,300.0, 0},
    {500.0,40.0, 0},
    {700.0,1000.0, 0},
    {300.0,200.0, 0},
  ...
  float rotation[3] = {40,20,325};
  int rotation_axis = 0;
  int preset = 0;
  int count = 0;
  float fudgeFactor = 10;

  float scale[3] = {3.0,3.0,3.0};
  int scale_forward = 1;
  int scale_axis = 0;
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    float tmp_matrix[16], tmp_matrix_2[16], tmp_matrix_3[16];
    ...
    makeZToWMatrix(fudgeFactor, tmp_matrix_2);
    multiply_4x4_mat(tmp_matrix_2, tmp_matrix, tmp_matrix_3);
    translate_4x4_mat(tmp_matrix_3, translation[preset][0], translation[preset][1], translation[preset][2], tmp_matrix_2);
