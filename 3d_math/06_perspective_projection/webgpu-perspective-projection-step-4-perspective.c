...
int main(int argc, char *argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    rotateX(tmp_matrix_2, degToRad(rotation[0]), tmp_matrix);
    rotateY(tmp_matrix, degToRad(rotation[1]), tmp_matrix_2);
    rotateZ(tmp_matrix_2, degToRad(rotation[2]), tmp_matrix);
