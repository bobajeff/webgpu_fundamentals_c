...
void aim(float * eye, float * target, float * up, float * dest) {

    float tmp_vector[3];
    float zAxis[3];
    subtract_vec3(target, eye, tmp_vector);
    normalize_vec3(tmp_vector, zAxis);
    float xAxis[3];
    cross_vec3(up, zAxis, tmp_vector);
    normalize_vec3(tmp_vector, xAxis);
...
int main(int argc, char *argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    float viewMatrix[16];
    lookAt(eye, target, up, viewMatrix);
    ...
    for (i = 0; i < numFs; ++i) {
        ...
        if (i < 25) {
            ...
            multiply_4x4_mat(viewProjectionMatrix, aimMatrix, uniformValues.matrix);
        } else {
            translate_4x4_mat(viewProjectionMatrix, settings_target[0], settings_target[1], settings_target[2], uniformValues.matrix);
        }
