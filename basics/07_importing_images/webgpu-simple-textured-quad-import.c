...
WGPUTexture createTextureFromImage(WGPUDevice device, char * path, int flipY) {
  int kTextureWidth, kTextureHeight, raw_image_channels;
  
  stbi_set_flip_vertically_on_load(flipY);
  unsigned char *textureData = stbi_load(path, &kTextureWidth, &kTextureHeight, &raw_image_channels, 4);
