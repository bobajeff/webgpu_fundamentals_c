...
circleVertices createCircleVertices(circleVerticesConfig config) {
  ...
  const int vertexDataSize = numVertices * sizeof(point_data);
  // malloc is used so I can create array at runtime and free it when I'm done with it
  point_data *vertexData = (point_data *)malloc(vertexDataSize);
  const circleVertices return_data = {.numVertices = numVertices,
  ...
  for (i = 0; i < config.numSubdivisions; ++i) {
    ...
    const float angle2 =
        config.startAngle + (i + 1) * (config.endAngle - config.startAngle) /
                                config.numSubdivisions;
    ...
    const char innerColor[3] = {(char) 1.0 * 255, (char) 1.0 * 255, (char) 1.0 * 255};
    const char outerColor[3] = {(char) 0.1 * 255, (char) 0.1 * 255, (char) 0.1 * 255};

    // first triangle
    vertexData[offset].x = c1 * config.radius;
    vertexData[offset].y = s1 * config.radius;
    vertexData[offset].r = outerColor[0];
    vertexData[offset].g = outerColor[1];
    vertexData[offset].b = outerColor[2];
    offset++;

    vertexData[offset].x = c2 * config.radius;
    vertexData[offset].y = s2 * config.radius;
    vertexData[offset].r = outerColor[0];
    vertexData[offset].g = outerColor[1];
    vertexData[offset].b = outerColor[2];
    offset++;

    vertexData[offset].x = c1 * config.innerRadius;
    vertexData[offset].y = s1 * config.innerRadius;
    vertexData[offset].r = innerColor[0];
    vertexData[offset].g = innerColor[1];
    vertexData[offset].b = innerColor[2];
    offset++;
    ...
    vertexData[offset].x = c2 * config.innerRadius;
    vertexData[offset].y = s2 * config.innerRadius;
    vertexData[offset].r = innerColor[0];
    vertexData[offset].g = innerColor[1];
    vertexData[offset].b = innerColor[2];
    offset++;
              ...
              (WGPUVertexState){
                  ...
                  .buffers =
                      &(WGPUVertexBufferLayout){
                          .arrayStride =
                              2 * 4 + 4, // 2 floats, 4 bytes each + 4 bytes color
                          .attributes =
                              (WGPUVertexAttribute[2]){
                                  {.shaderLocation = 0,
                                   .offset = 0,
  ...
  const WGPUBuffer vertexBuffer = wgpuDeviceCreateBuffer(
      device, &(WGPUBufferDescriptor){.label = "vertex buffer vertices",
                                      ...
                                      .usage = WGPUBufferUsage_Vertex |
                                               WGPUBufferUsage_CopyDst,
                                      .mappedAtCreation = false});
  wgpuQueueWriteBuffer(queue, vertexBuffer, 0, vertexValues.vertexData,
                       vertexValues.vertexDataSize);
  free(vertexValues
           .vertexData); // free the malloced data after it's loaded in gpu
