...
WGPUQueue queue = wgpuDeviceGetQueue(device);
  ...
  WGPUShaderModuleDescriptor shaderSource =
      load_wgsl(RESOURCE_DIR "/03_uniforms/webgpu-simple-triangle-uniforms-split.wgsl");
  ...
  const uint64_t staticUniformBufferSize =
  ...
  for (i = 0; i < kNumObjects; ++i) {
      ...
      const WGPUBuffer staticUniformBuffer = wgpuDeviceCreateBuffer(
      device, &(WGPUBufferDescriptor){
        .label = "static uniforms for obj: $i", // Note: C can't do string interpolation as easily as other languages. So I avoid that and just quote the variable '$i' instead.
    ...
    // These are only set once so set them now
      ...
      .color = {rand_(0, 0), rand_(0, 0), rand_(0, 0), 1.0}, // set the color
      .offset = {rand_(-0.9, 0.9), rand_(-0.9, 0.9)}        // set the offset
    };

    wgpuQueueWriteBuffer(queue, staticUniformBuffer, 0, &uniformValues, sizeof(uniformValues));
